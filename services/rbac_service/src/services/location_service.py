...
class LocationService:
    ...
    async def get_location(location_id: UUID) -> Location | None:
        ...
        async with session:
            query = select(Location).where(Location.id == str(location_id))
    ...
    async def list_locations(clinic_id: UUID = None) -> list[Location]:
        ...
        async with session:
            ...
            if clinic_id:
                query = query.where(Location.clinic_id == str(clinic_id))
            result = await session.execute(query)
    ...
    async def create_location(loc_data: LocationCreate) -> Location:
        ...
        async with session:
            db_location = Location(
                ...
                manager_id=str(loc_data.manager_id) if loc_data.manager_id else None,
                is_active=loc_data.is_active,
            )
            session.add(db_location)
            await session.commit()
            await session.refresh(db_location)
            _log.info(f"Created location: {db_location.id} for clinic {db_location.clinic_id}")
            return db_location

    @staticmethod
    async def update_location(location_id: UUID, loc_data: LocationUpdate) -> Location:
        session = await LocationService.get_session()
        async with session:
            query = select(Location).where(Location.id == str(location_id))
            result = await session.execute(query)
