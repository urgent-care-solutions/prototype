...
class ReportingService:
    ...
    async def ingest_appointment(msg: AppointmentCreated):
        async with AsyncSessionLocal() as session:
            ...
            exists = await session.get(ReportingAppointment, str(msg.id))
            if exists:
                return
    ...
    async def ingest_appointment_cancellation(msg: AppointmentCanceled):
        async with AsyncSessionLocal() as session:
            appt = await session.get(ReportingAppointment, str(msg.appointment_id))
            if appt:
    ...
    async def ingest_charge(msg: ChargeCreated):
        ...
        async with AsyncSessionLocal() as session:
            exists = await session.get(ReportingTransaction, str(msg.transaction_id))
            if exists:
                return
            ...
            session.add(tx)
            await session.commit()
            _log.info(f"Ingested charge {msg.transaction_id} for revenue analytics")
    ...
    async def ingest_patient_deletion(msg: PatientDeleted):
        async with AsyncSessionLocal() as session:
            pat = await session.get(ReportingPatient, str(msg.patient_id))
            if pat:
    ...
    @staticmethod
    async def get_revenue_stats(start: date | None, end: date | None) -> RevenueStats:
        async with AsyncSessionLocal() as session:
            filters = [ReportingTransaction.status == "success"]
            if start:
                filters.append(ReportingTransaction.transaction_date >= start)
            if end:
                filters.append(ReportingTransaction.transaction_date <= end)

            # Sum Charges
            charge_stmt = select(func.sum(ReportingTransaction.amount)).where(
                and_(*filters, ReportingTransaction.type == "CHARGE")
            )
            refund_stmt = select(func.sum(ReportingTransaction.amount)).where(
                and_(*filters, ReportingTransaction.type == "REFUND")
            )
            count_stmt = select(func.count(ReportingTransaction.id)).where(
                and_(*filters)
            )

            total_revenue = (await session.execute(charge_stmt)).scalar() or 0.0
            total_refunds = (await session.execute(refund_stmt)).scalar() or 0.0
    ...
    async def get_appointment_stats(
        ...
        async with AsyncSessionLocal() as session:
            ...
            # Total
            total_stmt = select(func.count(ReportingAppointment.id)).where(
                and_(*filters)
            )
            ...
            return AppointmentStats(total=total, by_status=by_status, by_type=by_type)

    @staticmethod
    async def get_patient_stats(start: date | None, end: date | None) -> PatientStats:
        async with AsyncSessionLocal() as session:
            # Total Active
            ...
            filters = []
            if start:
                filters.append(ReportingPatient.registration_date >= start)
            if end:
                filters.append(ReportingPatient.registration_date <= end)

            new_stmt = select(func.count(ReportingPatient.id)).where(and_(*filters))
            new_in_period = (await session.execute(new_stmt)).scalar() or 0
